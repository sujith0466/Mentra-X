...
from models import CodingChallenge, CodingSubmission, InterviewSession, ProjectIdea, SkillProgress, StudentProject
from services.ai.agents.interview_agent import InterviewAgent
from services.ai.agents.learning_agent import LearningAgent
from services.ai.learning.study_planner_service import generate_adaptive_learning_path
...
def generate_learning_feed(
    ...
    *,
    ...
    if learning_summary:
        feed_items.append(_format_feed_item("Learning AI suggests", _summarize_first_sentence(learning_summary)))
    ...
    if interview_summary:
        feed_items.append(_format_feed_item("Interview AI recommends", _summarize_first_sentence(interview_summary)))

    latest_challenge = CodingChallenge.query.order_by(CodingChallenge.created_at.desc(), CodingChallenge.id.desc()).first()
    if latest_challenge:
        feed_items.append(_format_feed_item("New coding challenge available", latest_challenge.title))
    ...
    if latest_interview:
        feed_items.append(f"Mock interview completed for {latest_interview.role} - Interview Update")
        feed_items.append(f"interview score improved to {int(latest_interview.score)}%")
    ...
    if latest_project_idea:
        feed_items.append(_format_feed_item("Project Idea", latest_project_idea.title))
    ...
    if latest_student_project and latest_student_project.project_idea:
        if latest_student_project.completed_at:
            feed_items.append(_format_feed_item("Project completed", latest_student_project.project_idea.title))
        elif latest_student_project.progress_percentage > 0:
            feed_items.append(
                _format_feed_item(
                    "Project progress updated",
                    latest_student_project.project_idea.title,
                    f"{int(latest_student_project.progress_percentage)}% complete",
                )
            )

    recommendations = recommend_courses(user_id)[:2]
    for item in recommendations:
        feed_items.append(_format_feed_item("Recommended Course", item["title"]))
    ...
    for topic in difficulty.get('weak_topics', [])[:2]:
        feed_items.append(_format_feed_item("Skill Focus", topic, "Weak topic detected"))

    adaptive_path = generate_adaptive_learning_path(user_id)
    for quiz_title in adaptive_path.get('recommended_quizzes', [])[:2]:
        feed_items.append(_format_feed_item("Practice Quiz", quiz_title))
    ...
    for item in snapshot_rows:
        feed_items.append(_format_feed_item("Skill Progress Update", item["skill_name"], f"{int(item['progress_percentage'])}%"))
    ...
    if latest_submission and latest_submission.challenge:
        ...
        if skill_row:
            ...
            feed_items.append(_format_feed_item(topic_label, f"{int(skill_row.progress_percentage)}%"))
    ...
    ideas = generate_project_ideas(domain)
    if ideas:
        feed_items.append(_format_feed_item("Project Idea", ideas[0]["title"]))
