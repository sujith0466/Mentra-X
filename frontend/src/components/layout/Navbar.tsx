...
export const Navbar: React.FC = () => {
  ...
  return (
    <header className="sticky top-0 z-40 w-full h-16 enterprise-glass border-b border-white/10 flex items-center justify-between px-6 transition-all">
      <div className="flex items-center space-x-4">
        ...
        <Link to="/" className="flex items-center space-x-2.5">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[10px] font-medium text-indigo-400 tracking-wider uppercase mt-0.5">
              Enterprise AI
            </span>
          </div>
        </Link>
        <div className="hidden md:flex items-center space-x-1 pl-6">
          <Badge variant="cyan" size="sm">
            <ShieldCheck className="w-3 h-3 mr-1" /> Enkrypt Layer Active
          </Badge>
        </div>
      </div>
      ...
      <div className="flex items-center space-x-3">
        <button
          onClick={toggleChat}
          className="relative inline-flex items-center space-x-2 px-3 py-1.5 rounded-lg bg-primary-500/10 border border-primary-500/20 text-indigo-300 hover:bg-primary-500/20 hover:text-white transition-all text-xs font-medium"
        >
          <MessageSquare className="w-3.5 h-3.5 text-indigo-400" />
          <span>Mastra AI Assistant</span>
        ...
        {user ? (
          <div className="flex items-center space-x-3 pl-3 border-l border-obsidian-600">
            <div className="flex items-center space-x-2">
              <div className="w-8 h-8 rounded-full bg-indigo-600/30 border border-indigo-500/40 flex items-center justify-center text-xs font-semibold text-indigo-200">
