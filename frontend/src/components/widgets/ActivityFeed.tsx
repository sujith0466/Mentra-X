    ...
    <Card variant="default" className={cn("p-5 space-y-4", className)}>
      ...
      <div className="space-y-3 max-h-80 overflow-y-auto pr-1">
        {items.map((it) => (
          <div
            key={it.id}
            className="flex items-start justify-between p-3 rounded-lg bg-obsidian-900/60 border border-obsidian-600/50 hover:border-indigo-500/30 transition-all text-xs"
          >
            <div className="flex items-start space-x-2.5">
              ...
              <div>
                <span className="font-semibold text-white">{it.user}</span>
                <p className="text-slate-300 mt-0.5 leading-snug">{it.action}</p>
              </div>
            </div>
