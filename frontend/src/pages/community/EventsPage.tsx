...
export const EventsPage: React.FC = () => {
  const [events, setEvents] = useState<EventItem[]>([
    {
      id: "ev-1",
      title: "Live Masterclass: Mastering AI Agents",
      speaker: "Mentra X Engineering Team",
      date: "July 15, 2026",
      time: "2:00 PM - 3:30 PM EST",
      location: "Virtual Webinar (Zoom)",
      isVirtual: true,
      attendeesCount: 342,
      description: "Explore advanced techniques for building and scaling AI agents for real-world applications.",
      isRSVPed: true,
      tag: "Masterclass",
    },
    {
      ...
      isVirtual: false,
      attendeesCount: 510,
      description: "Compete for $10,000 in cloud credits by building innovative AI-powered applications.",
      isRSVPed: false,
      tag: "Hackathon",
    },
    {
      id: "ev-3",
      title: "AI Security & Compliance Panel",
      speaker: "Mentra X AI Safety Team",
      date: "August 5, 2026",
      time: "1:00 PM - 2:00 PM EST",
      location: "Virtual Stream",
      isVirtual: true,
      attendeesCount: 188,
      description: "A deep dive into data privacy, AI safety guidelines, and security protocols in production applications.",
      isRSVPed: false,
      tag: "Panel",
    },
  ]);
    ...
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 dark:border-white/10 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white">Upcoming Events & Live Workshops</h1>
      ...
      <div className="space-y-6">
        {events.map((ev) => (
          <Card key={ev.id} variant={ev.isRSVPed ? "glow" : "default"} className="p-6 sm:p-8 transition-all hover:border-slate-300 dark:border-white/20">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
              <div className="space-y-3 flex-1">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white">{ev.title}</h3>
                <p className="text-sm text-slate-600 dark:text-slate-300 leading-relaxed max-w-3xl font-sans">{ev.description}</p>

                <div className="flex flex-wrap items-center gap-6 pt-2 text-xs text-slate-600 dark:text-slate-300">
              ...
              <div className="shrink-0 flex flex-col sm:flex-row lg:flex-col items-start sm:items-center lg:items-end justify-between gap-3 pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-200 dark:border-white/10">
                {ev.isRSVPed && (
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Host an Academic Workshop">
        <form onSubmit={handleCreateEvent} className="space-y-4 text-slate-600 dark:text-slate-300">
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1">Event Title</label>
            <Input placeholder="e.g., Intro to Transformers Lab Session" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} required />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1">Speaker / Host Name</label>
            <Input placeholder="e.g., Prof. Sarah Jenkins" value={newSpeaker} onChange={(e) => setNewSpeaker(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1">Date & Time</label>
            <Input placeholder="e.g., July 30, 2026 at 4:00 PM EST" value={newDate} onChange={(e) => setNewDate(e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-slate-600 dark:text-slate-300 mb-1">Description & Prerequisites</label>
            <textarea
              rows={4}
