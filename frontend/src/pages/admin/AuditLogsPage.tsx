...
export const AuditLogsPage: React.FC = () => {
  const logs: AuditRow[] = [
    { id: "LOG-9001", timestamp: "2026-07-06 06:35:12", source: "Enkrypt Layer 6 Gateway", action: "Intercepted off-topic query in Agentic Coding tutor", status: "HARD_FAIL", user: "STU-103", similarity: 0.42 },
    { id: "LOG-9002", timestamp: "2026-07-06 06:34:00", source: "Mastra Swarm Router", action: "Routed student query to Qdrant vector memory search tool", status: "VERIFIED", user: "STU-101", similarity: 0.99 },
    { id: "LOG-9003", timestamp: "2026-07-06 06:30:45", source: "Enkrypt Safety Interceptor", action: "Regenerating tutoring response due to low confidence threshold", status: "REGENERATE", user: "STU-104", similarity: 0.68 },
    { id: "LOG-9004", timestamp: "2026-07-06 06:25:10", source: "Digital Twin Sync Service", action: "Updated knowledge retention heuristics for 1,240 twins", status: "APPROVE", user: "SYSTEM_SWARM", similarity: 1.0 },
  ];

  const columns: Column<AuditRow>[] = [
    { key: "id", title: "Event ID", sortable: true, render: (item) => <span className="font-mono text-xs text-slate-400">{item.id}</span> },
    { key: "timestamp", title: "Timestamp", sortable: true, render: (item) => <span className="font-mono text-xs text-slate-300">{item.timestamp}</span> },
    { key: "source", title: "Source Component", sortable: true, render: (item) => <span className="font-semibold text-white">{item.source}</span> },
    { key: "action", title: "Action Description", render: (item) => <span className="text-slate-300">{item.action}</span> },
    { key: "status", title: "Enkrypt Verdict", sortable: true, render: (item) => <SafetyBadge status={item.status} score={item.similarity} /> },
    ...
    <div className="space-y-8 py-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-obsidian-600 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white mt-1">Security Audit & Governance Logs</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card variant="glass" className="p-4 flex items-center justify-between">
          <div>
            ...
            <span className="text-2xl font-bold text-white mt-1 block">142 Events</span>
          </div>
