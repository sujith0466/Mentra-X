...
export const ManageCoursesPage: React.FC = () => {
  const courses: CourseRow[] = [
    ...
    { id: "ml-303", title: "Natural Language Processing", category: "AI & Machine Learning", modules: 10, students: 280, aiTutor: "Enabled", status: "Published" },
  ...
  const columns: Column<CourseRow>[] = [
    ...
    { key: "title", title: "Course Title", sortable: true, render: (item) => <span className="font-bold text-slate-900 dark:text-white">{item.title}</span> },
    { key: "category", title: "Domain", sortable: true, render: (item) => <Badge variant="purple" size="sm">{item.category}</Badge> },
    { key: "modules", title: "Syllabus", sortable: true, render: (item) => <span className="text-slate-600 dark:text-slate-300">{item.modules} Modules</span> },
    ...
    { key: "aiTutor", title: "AI Tutor", render: (item) => <Badge variant="cyan" size="sm">{item.aiTutor}</Badge> },
    ...
    <div className="space-y-8 py-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 dark:border-obsidian-600 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white mt-1">Syllabus & Course Catalog Management</h1>
